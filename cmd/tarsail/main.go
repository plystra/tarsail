...
import (
	...
	"github.com/plystra/tarsail/internal/localcmd"
...
func (a *app) dockerRunnerForRelease(project *config.Project, releaseID string) localdocker.Runner {
	return localdocker.Runner{
		...
		Stderr:  a.stderr,
		Verbose: a.verbose,
		Env:     releaseEnv(releaseID),
	}
}

func (a *app) localRunnerForRelease(releaseID string) localcmd.Runner {
	return localcmd.Runner{
		Stdout: a.stdout,
		Stderr: a.stderr,
		Env:    releaseEnv(releaseID),
	}
...
func releaseEnv(releaseID string) []string {
	var env []string
	if releaseID != "" {
...
func (a *app) cmdDoctor(ctx context.Context) error {
	...
	releaseFilesLabel := "release file sources available"
	if project.HasBuildSteps() {
		releaseFilesLabel = "release file sources available or generated by build steps"
	}
	if err := checkManagedFilesForDoctor(project); err != nil {
		ui.Fail(a.stdout, releaseFilesLabel)
		fmt.Fprintln(a.stderr, err)
		localErrs++
	} else {
		ui.OK(a.stdout, releaseFilesLabel)
	}
...
func checkManagedFilesForDoctor(project *config.Project) error {
	...
	return fmt.Errorf("[config:files] Release file source is missing: %s\n\nHow to fix:\n  Create the file or directory before deploy, or add build.steps to generate it before Tarsail creates the release bundle.", strings.Join(missing, ", "))
}
...
func (a *app) cmdDeploy(ctx context.Context) error {
	...
	ui.Step(a.stdout, step, totalSteps, "Checking local environment")
	...
	if project.HasBuildSteps() {
		ui.Step(a.stdout, step, totalSteps, "Running build steps")
		if err := a.runBuildSteps(ctx, project, releaseID); err != nil {
			return err
		}
		step++
	}
	...
	ui.Step(a.stdout, step, totalSteps, "Checking remote server")
	if err := client.CheckSSH(ctx); err != nil {
		return err
	}
	if err := client.EnsureLayout(ctx); err != nil {
	...
	ui.Step(a.stdout, step, totalSteps, "Building images")
	...
	ui.Step(a.stdout, step, totalSteps, "Creating bundle")
	...
	ui.Step(a.stdout, step, totalSteps, "Uploading bundle")
	...
	ui.Step(a.stdout, step, totalSteps, "Loading images")
	if err := client.LoadImages(ctx, releaseID); err != nil {
		return err
	}
	step++

	ui.Step(a.stdout, step, totalSteps, "Starting Compose app")
	...
	ui.Step(a.stdout, step, totalSteps, "Reading status")
...
func (a *app) runBuildSteps(ctx context.Context, project *config.Project, releaseID string) error {
	runner := a.localRunnerForRelease(releaseID)
	for index, step := range project.Build.Steps {
		name := step.Name
		if name == "" {
			name = fmt.Sprintf("step %d", index+1)
		}
		fmt.Fprintln(a.stdout, "  - "+name)
		if err := runner.Run(ctx, "build:step", name, step.Run, project.BuildStepDir(step)); err != nil {
			return err
		}
	}
	return nil
...
func checkManagedFilesForDeploy(project *config.Project) error {
	missing, err := project.MissingManagedFileSources()
	if err != nil {
		return err
	}
	...
	return fmt.Errorf("[config:files] Release file source is missing after build steps: %s\n\nWhy it matters:\n  Tarsail copies files[].source into the release bundle after local build steps run.\n\nHow to fix:\n  Check build.steps and files[].source. For a Vite web app, run the web build before bundling and point files[].source at the generated dist directory.", strings.Join(missing, ", "))
}
